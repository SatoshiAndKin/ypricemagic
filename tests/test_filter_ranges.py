...
async def test_scan_refills_consumed_slots_before_the_whole_window_finishes() -> None:
    ...
    try:
        ...
        # Only index zero can be consumed while index one is pending. Finished
        # later results still occupy their slots in the bounded scan window.
        assert len(calls) == window_size + 1
        assert state._extend.await_args_list[0].args == ([0],)
    finally:
        release.set()
        await asyncio.wait_for(task, 1)
    assert [call.args[0] for call in state._extend.await_args_list] == [
    ...
    assert state._set_lock.await_args.args == (999,)
