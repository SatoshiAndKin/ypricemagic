...
async def redeem(
    ...
    async def output(underlying: str, amount: int) -> QuoteAsset:
        ...
        decimals = (
            ...
            else int(await read(underlying, "decimals()(uint8)", block))
        )
        return QuoteAsset(underlying, amount, decimals)
    ...
    if version in ("0.4.3", "0.4.4", "0.4.5", "0.4.6"):
        ...
        if amount > int(idle):
            return None  # Strategy withdrawals can realize unmodeled losses.
        return await one(
            f"Yearn V2 {version}",
            underlying,
            amount,
